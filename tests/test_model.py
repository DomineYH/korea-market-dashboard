...
from korea_market_dashboard.model import build_market_prediction_tables, build_prediction_table, classify_score
...
def sample_snapshot():
    return {
        "investor_flows_억원": {
            ...
            "KOSDAQ": {
                "sums_억원": {
                    "5d": {"foreign": -1428, "individual": 5704, "institution_total": -3163},
                    "10d": {"foreign": 10126, "individual": 5551, "institution_total": -13332},
                }
            },
        },
        "yahoo_market": {
            "KOSPI": {"1m_change_pct": 25.56, "3m_change_pct": 41.43, "6m_change_pct": 89.52},
            "KOSDAQ": {"1m_change_pct": 0.71, "3m_change_pct": 0.2, "6m_change_pct": 28.86},
...
class PredictionModelTests(unittest.TestCase):
    ...
    def test_build_prediction_table_scores_known_signals_from_snapshot(self):
        result = build_prediction_table(sample_snapshot())
    ...
    def test_build_market_prediction_tables_returns_separate_kospi_and_kosdaq_models(self):
        result = build_market_prediction_tables(sample_snapshot())

        self.assertEqual(set(result), {"KOSPI", "KOSDAQ"})
        self.assertTrue(all("total_score" in model for model in result.values()))
        self.assertTrue(any(row["factor"].startswith("KOSDAQ") for row in result["KOSDAQ"]["signals"]))
        self.assertNotEqual(result["KOSPI"]["total_score"], result["KOSDAQ"]["total_score"])
